# Hi, I'm Wagner da Rocha 👋

🔬 Postdoctoral researcher at [IMECC](https://www.ime.unicamp.br/) / [UNICAMP](https://unicamp.br/), working on distance geometry, molecular structure reconstruction, and scientific computing.

## Research interests 📚

- Distance geometry
- Molecular distance geometry problems
- Protein structure determination
- Algorithms for inverse problems
- Scientific software and computational experiments

## Selected repositories 💻

- [bpbm-3d-iddgp](https://github.com/wdarocha/bpbm-3d-iddgp)  
  C implementation of BP-based methods for the 3D interval discretizable distance geometry problem.

- [pdb-parser](https://github.com/wdarocha/pdb-parser)  
  Python toolkit for processing PDB structures and generating distance-geometry instances.

- [benchmarks](https://github.com/wdarocha/benchmarks)  
  Benchmark and experiment data associated with my research repositories.

## Links 🔗

- [Personal website](https://ime.unicamp.br/~wdarocha)
- [Google Scholar](https://scholar.google.com/citations?user=4s1PiuAAAAAJ&hl)
